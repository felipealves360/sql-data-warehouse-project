...
BEGIN
		 ...
		 cst_create_date)
			   SELECT cst_id,
					  ...
					  CASE UPPER( TRIM( cst_marital_status ) )
              			...
              			WHEN 'M' THEN 'Married'
              			ELSE 'n/a'
					  END AS cst_marital_status, -- Normalize marital status to readble format 
					  CASE UPPER( TRIM( cst_gndr ) )
              			WHEN 'F' THEN 'Female'
              			WHEN 'M' THEN 'Male'
              			ELSE 'n/a'
					  END AS cst_gndr, -- Normalize gender values to readble
					  cst_create_date
			   FROM (SELECT *,
		 ...
		 prd_end_dt)
			   SELECT prd_id,
					  ...
					  CASE UPPER( TRIM( prd_line ) )
						   ...
						   WHEN 'T' THEN 'Touring'
						   ELSE 'n/a'
					  END AS prd_line, -- Map product line with descreptive names
					  CAST( prd_start_dt AS DATE ) AS prd_start_dt, -- Convert datetime for date
					  CAST( LEAD( prd_start_dt ) OVER (PARTITION BY prd_key ORDER BY prd_start_dt ASC)-1 AS DATE ) AS prd_end_dt -- Calculate the end date as one day before the next star_date
	...
	SET @start_time = GETDATE( )
		PRINT '>> Truncating data into: silver.crm_sls_details'
		 ...
		 sls_price)
			   SELECT sls_ord_num,
					  ...
					  CASE
						   ...
						   ELSE sls_price
					  END AS sls_price -- Derive price original values is invalid
		...
		(cid,
		 ...
		 gen)
			   SELECT CASE
					  ...
					  END AS cid,
					  CASE
						   WHEN bdate>GETDATE( ) THEN NULL -- correcting of birthdates in the future
						   ELSE bdate
					  END AS bdate,
					  CASE
